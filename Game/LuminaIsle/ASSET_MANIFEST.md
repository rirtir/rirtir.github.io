...
- [x] `assets/asset-metrics.js` / `.json` — 134画像のアルファ境界、可視中心、足元基準
...
- [x] `assets/combat-world-v5.png` — 新通常敵3種、回収バッグ、飛び道具3種、祭壇の4×2生成シート
- [x] 全読込用PNGへクロマキー除去、ソフトマット、デスピル処理
- [x] 文字、透かし、第三者ロゴ、欠落を目視監査
- [x] 連結画素を所属セルへ割り当て、別セルを透明化。独立PNG134点の端接触0

`*-source.png` は生成原本、末尾が `-v3.png` のシートは分離元の透過版。既存素材は実行時に `generated-v4/` の独立画像を使う。P11追加分だけは固定4×2シートを一度読み込み、端から連結する無彩色背景をCanvas上で除去して各セルを描く。全シートのセル順と最終プロンプト、地表生成プロンプトは `assets/IMAGE_PROMPT.md` に記録する。
...
- [x] `tools/audit_assets.py` — 透明余白を監査し、可視領域ベースの描画補正データを生成
...
- [x] P8キャラクター・物品画像、P9地表画像、P11戦闘・世界追加画像はOpenAI built-in imagegenの通常生成
