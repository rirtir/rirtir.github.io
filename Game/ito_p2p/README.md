...
2. 「参加者を追加」で招待画面を表示する。招待リンクを参加者に送る（コピーまたは共有ボタン）か、招待QRを読み取ってもらう。
3. 参加端末は招待リンクを開いて表示名を入力し「参加する」を押す。リンクが使えない場合は「招待QRを読み取る」を押してQRを読み取る。
4. 参加端末に表示された回答QRを、ホストの「回答QRを読む」で読み取る。
5. 人数分だけ2〜4を繰り返し、ホストがゲームを開始する。

招待リンクを使う場合、参加端末はQRを読み取らずに済みます（往復が必要な回答だけQRが残ります）。
...
- QRコード / 招待リンク: WebRTC Offer / Answer / ICE候補の手動シグナリング。招待（Offer）はURLの `#i=` フラグメントにも埋め込めるため、参加端末はリンクを開くだけでQRを読み取らずに済む
- 接続コードはBase45でエンコードしてQRのAlphanumericモードに載せている（Base64+Byteモードより1枚のQRに収まりやすい）
